...
void processing(double input[][BLOCK_SIZE], double output[][BLOCK_SIZE], double gain, int mode)
{
	...
	for (int i = 0; i < BLOCK_SIZE; i++)
	{
		...
		output[L_CHANNEL][i] = input_L_with_gain;	// L channel
		output[LS_CHANNEL][i] = -input_L_with_gain;		// LS is inverted
		output[LS_CHANNEL][i] = second_order_IIR(output[LS_CHANNEL][i], HPF5kHz, x_historyLs, y_historyLs);
		output[R_CHANNEL][i] = input_R_with_gain;		// R channel
		...
		if (mode == OM_3_2_0 || mode == OM_3_2_1)
		{
			...
			output[C_CHANNEL][i] = saturation(input_L_with_stage_two_gain + input_R_with_stage_two_gain, limiterThreshold);
		}
		...
		if (mode == OM_2_2_1 || mode == OM_3_2_1)
		{
			...
			output[LFE_CHANNEL][i] = input_L_with_stage_two_gain - input_R_with_stage_two_gain;
			output[LFE_CHANNEL][i] = saturation(output[LFE_CHANNEL][i], limiterThreshold);
			output[LFE_CHANNEL][i] = second_order_IIR(output[LFE_CHANNEL][i], LPF800Hz, x_historyLfe, y_historyLfe);
